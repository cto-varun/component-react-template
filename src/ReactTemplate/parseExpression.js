...
const parseFunction = (options, expression, returnsOptionsParam) => {
    try {
        return new Function(
            argumentType(options, returnsOptionsParam),
            `return ${expression}`
        )(options);
    } catch (e) {
...
const initializeParserKeys = (id, config) => {
    ...
    return {
        ...
        PARSE_CHILD: parseKeyExpression()(
            parseKey,
            overrideParseChild || ((k) => `${k}child`)
        ),
        PARSE_JSX: parseKeyExpression()(
            parseKey,
            overrideParseJSX || ((k) => `${k}jsx`)
        ),
        INSTANCE_ID: id || '',
    };
};
 ...
 * @param {} parserTokens
 */
export const parseEntity = (parserTokens, children) => {
    const {
        PARSE_KEY,
        FUNCTION_START,
    ...
    const parser = (exp, options) => {
        ...
        if (expression.includes(PARSE_KEY)) {
            ...
            return getOrHas(options, 'get')(property);
        }
...
const parseExpression = (id, config, children) => {
    ...
    const expressionIterator = (options) => (expression) => {
        const expType = prim(expression);

        if (expType === 'array') {
            return expression.map((v) => expressionIterator(options)(v));
        }
        if (expType === 'object') {
            return mapValues(expression, expressionIterator(options));
        }
